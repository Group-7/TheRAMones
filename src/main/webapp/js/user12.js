...
$("#chart-area").click( 
	    function(evt){
	       ...
	       // var url = "label= " + activePoints[0].label + "&value= " + activePoints[0].value;
            //alert(url);
            getDetailsForImsi(activePoints[0].label);
	    }
); 

function getDetailsForImsi(imsi){
	...
	$.ajax({
		type:"GET",
		url:"rest/baseData/toptenimsiDetails?startDate="+ startDate +"&endDate="+ endDate
			+"&imsi="+imsi,
		success: failureDetailsTable,
		contentType: 'application/json'
	});
...
function failureDetailsTable(data){
	...
	var ctx = document.getElementById("drill1").getContext("2d");
	window.myDrill = new Chart(ctx).Doughnut(doughnutData, {responsive : true});
